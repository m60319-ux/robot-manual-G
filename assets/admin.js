...
async function openImageGallery(targetRow) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div style="background: white; padding: 20px; border-radius: 8px; width: 80%; max-height: 80%; overflow-y: auto; position: relative;">
                <h3 style="margin-top:0;">📂 選擇圖片 (${config.name})</h3>
    ...
    try {
        if (currentMode === 'local' && localHandle) {
            ...
            try {
                ...
                contentDiv.innerHTML = `<p style="color:red">無法讀取資料夾 (assets/${config.name}/images): ${err.message}</p>`;
                return;
            }
    ...
    images.forEach(img => {
        ...
        `;
        item.onclick = () => {
            insertImageToRow(targetRow, img.url);
